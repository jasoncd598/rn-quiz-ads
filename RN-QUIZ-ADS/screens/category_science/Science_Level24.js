...
const Science_Level24 = ({ navigation }) => {
    ...
    useEffect(() => {
       async function init(){
            try {
                await AsyncStorage.setItem('@userLevelScience', '24')
            } catch (e) {
                console.log(`Error ${e}`)
            }
    ...
    <View style={styles.container}>
        <Appbar.Header style={{ backgroundColor:'#363A66'}}>
            ...
            <Appbar.Content title="Science - Level 24" subtitle={'Subtitle'} color='#fff'/>
        </Appbar.Header>

        <View style={styles.subContainer}>
            <View style={{ margin: 25, alignItems:'center', justifyContent:'center'}}>
                <Text style={{ fontSize:18, fontWeight:'900', color:'#2E2E2E'}}>What type of bond holds a water molecule together?</Text>
            </View>
            <View style={{ margin: 25, alignItems:'center', justifyContent:'center'}}>
                <TouchableOpacity style={styles.choiceButton} onPress={() => { checkAnwer('A') }}>
                    <Text style={styles.choiceText}>Covalent bond</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.choiceButton} onPress={() => { checkAnwer('B') }}>
                    <Text style={styles.choiceText}>Ionic bond</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.choiceButton} onPress={() => { checkAnwer('C') }}>
                    <Text style={styles.choiceText}>Hydrogen bond</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.choiceButton} onPress={() => { checkAnwer('D') }}>
                    <Text style={styles.choiceText}>Metallic bond</Text>
                </TouchableOpacity>
           </View>
        </View>

        {
            ...
            <View style={styles.modalChecker}>
                <TouchableOpacity style={styles.modalChecker} onPress={() => { navigation.push('Science_Level25Screen') }}>
                    <View>
                    ...
                    <View style={{ margin: 25, justifyContent:'center', alignItems:'center' }}>
                        <Text style={{ color:'#747474', textAlign:'center'}}>A water molecule is held together by a covalent bond between the two hydrogen atoms and the oxygen atom. Additionally, the molecule is held together by hydrogen bonds between the oxygen atoms of one water molecule and the hydrogen atoms of another</Text>
                    </View>
...
export default Science_Level24
