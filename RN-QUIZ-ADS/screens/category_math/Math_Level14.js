...
const Math_Level14 = ({ navigation }) => {
    ...
    useEffect(() => {
       async function init(){
            try {
                await AsyncStorage.setItem('@userLevelMath', '14')
            } catch (e) {
                console.log(`Error ${e}`)
            }
    ...
    <View style={styles.container}>
        <Appbar.Header style={{ backgroundColor:'#363A66'}}>
            ...
            <Appbar.Content title="Math - Level 14" subtitle={'Subtitle'} color='#fff'/>
        </Appbar.Header>

        <View style={styles.subContainer}>
            <View style={{ margin: 25, alignItems:'center', justifyContent:'center'}}>
                <Text style={{ fontSize:18, fontWeight:'900', color:'#2E2E2E'}}>A bag contains 4 red balls, 5 blue balls, and 6 green balls. If a ball is randomly selected from the bag, what is the probability that it is not blue?</Text>
            </View>
            <View style={{ margin: 25, alignItems:'center', justifyContent:'center'}}>
                <TouchableOpacity style={styles.choiceButton} onPress={() => { checkAnwer('A') }}>
                    <Text style={styles.choiceText}>2/3</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.choiceButton} onPress={() => { checkAnwer('B') }}>
                    <Text style={styles.choiceText}>5/9</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.choiceButton} onPress={() => { checkAnwer('C') }}>
                    <Text style={styles.choiceText}>11/15</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.choiceButton} onPress={() => { checkAnwer('D') }}>
                    <Text style={styles.choiceText}>9/15</Text>
                </TouchableOpacity>
           </View>
        </View>

        {
            ...
            <View style={styles.modalChecker}>
                <TouchableOpacity style={styles.modalChecker} onPress={() => { navigation.push('Math_Level15Screen') }}>
                    <View>
                    ...
                    <View style={{ margin: 25, justifyContent:'center', alignItems:'center' }}>
                        <Text style={{ color:'#747474', textAlign:'center'}}>There are a total of 15 balls in the bag (4 red + 5 blue + 6 green). Out of them only 5 are blue, so the probability of not picking a blue ball is (4+6)/15 = 11/15</Text>
                    </View>
