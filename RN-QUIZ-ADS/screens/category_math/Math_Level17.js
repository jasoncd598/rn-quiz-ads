...
const Math_Level17 = ({ navigation }) => {
    ...
    useEffect(() => {
       async function init(){
            try {
                await AsyncStorage.setItem('@userLevelMath', '17')
            } catch (e) {
                console.log(`Error ${e}`)
            }
    ...
    <View style={styles.container}>
        <Appbar.Header style={{ backgroundColor:'#363A66'}}>
            ...
            <Appbar.Content title="Math - Level 17" subtitle={'Subtitle'} color='#fff'/>
        </Appbar.Header>

        <View style={styles.subContainer}>
            <View style={{ margin: 25, alignItems:'center', justifyContent:'center'}}>
                <Text style={{ fontSize:18, fontWeight:'900', color:'#2E2E2E'}}>The equation of a parabola is given as y = x^2 - 6x + 8. What are the coordinates of the vertex of this parabola?</Text>
            </View>
            <View style={{ margin: 25, alignItems:'center', justifyContent:'center'}}>
                <TouchableOpacity style={styles.choiceButton} onPress={() => { checkAnwer('A') }}>
                    <Text style={styles.choiceText}>(3,2)</Text>
                </TouchableOpacity>
...

        {
            ...
            <View style={styles.modalChecker}>
                <TouchableOpacity style={styles.modalChecker} onPress={() => { navigation.push('Math_Level18Screen') }}>
                    <View>
                    ...
                    <View style={{ margin: 25, justifyContent:'center', alignItems:'center' }}>
                        <Text style={{ color:'#747474', textAlign:'center'}}>The vertex of a parabola in the form of y = a(x-h)^2 + k is given by the point (h,k). To find h and k, we can complete the square by adding (6/2)^2 = 9 to the constant term in the equation, giving us y = x^2 - 6x + 8 + 9 = (x-3)^2 + 2. The coordinates of the vertex are therefore (3,2)</Text>
                    </View>
