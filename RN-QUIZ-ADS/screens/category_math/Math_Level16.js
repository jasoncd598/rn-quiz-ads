...
const Math_Level16 = ({ navigation }) => {
    ...
    useEffect(() => {
       async function init(){
            try {
                await AsyncStorage.setItem('@userLevelMath', '16')
            } catch (e) {
                console.log(`Error ${e}`)
            }
    ...
    <View style={styles.container}>
        <Appbar.Header style={{ backgroundColor:'#363A66'}}>
            ...
            <Appbar.Content title="Math - Level 16" subtitle={'Subtitle'} color='#fff'/>
        </Appbar.Header>

        <View style={styles.subContainer}>
            <View style={{ margin: 25, alignItems:'center', justifyContent:'center'}}>
                <Text style={{ fontSize:18, fontWeight:'900', color:'#2E2E2E'}}>A coin is flipped twice. What is the probability of getting two heads?</Text>
            </View>
            <View style={{ margin: 25, alignItems:'center', justifyContent:'center'}}>
                <TouchableOpacity style={styles.choiceButton} onPress={() => { checkAnwer('A') }}>
                    <Text style={styles.choiceText}>0.25</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.choiceButton} onPress={() => { checkAnwer('B') }}>
                    <Text style={styles.choiceText}>0.5</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.choiceButton} onPress={() => { checkAnwer('C') }}>
                    <Text style={styles.choiceText}>0.75</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.choiceButton} onPress={() => { checkAnwer('D') }}>
                    <Text style={styles.choiceText}>0.9</Text>
                </TouchableOpacity>
           </View>
        </View>

        {
            ...
            <View style={styles.modalChecker}>
                <TouchableOpacity style={styles.modalChecker} onPress={() => { navigation.push('Math_Level17Screen') }}>
                    <View>
                    ...
                    <View style={{ margin: 25, justifyContent:'center', alignItems:'center' }}>
                        <Text style={{ color:'#747474', textAlign:'center'}}>The probability of an event is the number of favorable outcomes divided by the number of possible outcomes. In this case, there are 2 possible outcomes for each flip (heads or tails) and since the events are independent the total number of possible outcomes is 2*2 = 4. The only favorable outcome is getting two heads, so the probability of getting two heads is 1/4 or 0.25</Text>
                    </View>
