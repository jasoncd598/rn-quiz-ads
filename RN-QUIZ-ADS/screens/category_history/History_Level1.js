...
const History_Level1 = ({ navigation }) => {
    ...
    useEffect(() => {
        async function init(){
            try {
                await AsyncStorage.setItem('@userLevelHistory', '1')
            } catch (e) {
                console.log(`Error ${e}`)
            }
  ...
  return (
    <View style={styles.container}>
        <Appbar.Header style={{ backgroundColor:'#FF8B13'}}>
            ...
            <Appbar.Content title="History - Level 1" subtitle={'Subtitle'} color='#fff'/>
        </Appbar.Header>
...

        {
            ...
            <View style={styles.modalChecker}>
                <TouchableOpacity style={styles.modalChecker} onPress={() => { navigation.push('History_Level2Screen') }}>
                    <View>
