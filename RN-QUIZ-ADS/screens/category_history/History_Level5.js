...
const History_Level5 = ({ navigation }) => {
    ...
    useEffect(() => {
       async function init(){
            try {
                await AsyncStorage.setItem('@userLevelHistory', '5')
            } catch (e) {
                console.log(`Error ${e}`)
            }
    ...
    <View style={styles.container}>
        <Appbar.Header style={{ backgroundColor:'#FF8B13'}}>
            ...
            <Appbar.Content title="History - Level 5" subtitle={'Subtitle'} color='#fff'/>
        </Appbar.Header>
...

        {
            ...
            <View style={styles.modalChecker}>
                <TouchableOpacity style={styles.modalChecker} onPress={() => { navigation.push('History_Level6Screen') }}>
                    <View>
