...
const History_Level2 = ({ navigation }) => {
    ...
    useEffect(() => {
       async function init(){
            try {
                await AsyncStorage.setItem('@userLevelHistory', '2')
            } catch (e) {
                console.log(`Error ${e}`)
            }
    ...
    <View style={styles.container}>
        <Appbar.Header style={{ backgroundColor:'#FF8B13'}}>
            ...
            <Appbar.Content title="History - Level 2" subtitle={'Subtitle'} color='#fff'/>
        </Appbar.Header>
...

        {
            ...
            <View style={styles.modalChecker}>
                <TouchableOpacity style={styles.modalChecker} onPress={() => { navigation.push('History_Level3Screen') }}>
                    <View>
